...
def clean_data(gt_anno, dt_anno, current_class, difficulty, calib):
    ...
    for i in range(num_gt):
        ...
        ) and False:  # todo: remove this to enable difficulty changes
    ...
    for i in range(num_dt):
        ...
        if height < MIN_HEIGHT[difficulty] and False:
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
        else:
...
def bev_box_overlap(boxes, qboxes, gt_boxes_lidar, dt_boxes_lidar, criterion=-1):
    overlap = iou3d_nms_utils.boxes_bev_iou_cpu(
        gt_boxes_lidar[:, 0:7], dt_boxes_lidar[:, 0:7]
    )
...
def d3_box_overlap(boxes, qboxes, gt_boxes_lidar, dt_boxes_lidar, criterion=-1):
    gt_boxes_lidar_cuda = torch.from_numpy(gt_boxes_lidar).float().cuda()
    dt_boxes_lidar_cuda = torch.from_numpy(dt_boxes_lidar).float().cuda()

    overlap = (
        iou3d_nms_utils.boxes_iou3d_gpu(gt_boxes_lidar_cuda, dt_boxes_lidar_cuda)
        .cpu()
        .detach()
        .numpy()
    )
...
def calculate_iou_partly(gt_annos, dt_annos, metric, calib, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            # todo: add a 3d gt_bbox
            loc_3d = np.concatenate([a["location"] for a in gt_annos_part], 0)
            dims_3d = np.concatenate([a["dimensions"] for a in gt_annos_part], 0)
            rots_3d = np.concatenate([a["rotation_y"] for a in gt_annos_part], 0)
            loc_lidar = calib.rect_to_lidar(loc_3d)
            l, h, w = dims_3d[:, 0:1], dims_3d[:, 1:2], dims_3d[:, 2:3]
            loc_lidar[:, 2] += h[:, 0] / 2
            gt_boxes_lidar = np.concatenate(
            ...
            # todo: add a 3d dt_bbox
            loc_3d = np.concatenate([a["location"] for a in dt_annos_part], 0)
            dims_3d = np.concatenate([a["dimensions"] for a in dt_annos_part], 0)
            rots_3d = np.concatenate([a["rotation_y"] for a in dt_annos_part], 0)
            loc_lidar = calib.rect_to_lidar(loc_3d)
            l, h, w = dims_3d[:, 0:1], dims_3d[:, 1:2], dims_3d[:, 2:3]
            loc_lidar[:, 2] += h[:, 0] / 2
            ...
            overlap_part = bev_box_overlap(
                gt_boxes, dt_boxes, gt_boxes_lidar, dt_boxes_lidar
            ).astype(np.float64)
        elif metric == 2:
...
def eval_class(
    ...
    rets = calculate_iou_partly(dt_annos, gt_annos, metric, calib, num_parts)
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            # print("gt_datas_list from _prepare_data = ", gt_datas_list)
            # print("dt_datas_list from _prepare_data = ", dt_datas_list)
            # print("ignored_gts from _prepare_data = ", ignored_gts)
            # print("ignored_dets from _prepare_data = ", ignored_dets)
            # print("dontcares from _prepare_data = ", dontcares)
            # print("total_dc_num from _prepare_data = ", total_dc_num)
            # print("total_num_valid_gt from _prepare_data = ", total_num_valid_gt)
...
def do_eval(
    ...
    ret = eval_class(
        gt_annos, dt_annos, current_classes, difficultys, 1, min_overlaps, calib
    )
    ...
    ret = eval_class(
        gt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps, calib
    )
...
def do_coco_style_eval(
    gt_annos, dt_annos, current_classes, overlap_ranges, compute_aos, calib
):
    ...
    mAP_bbox, mAP_bev, mAP_3d, mAP_aos = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, calib, compute_aos
    )
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_aos:
                ...
                if i == 0:
                    ...
                    ret_dict[
                        "%s_aos/moderate_R40" % class_to_name[curcls]
                    ] = mAPaos_R40[j, 1, 0]
            ...
            if i == 0:
                ...
                ret_dict["%s_3d/moderate_R40" % class_to_name[curcls]] = mAP3d_R40[
                    j, 1, 0
                ]
                ret_dict["%s_3d/hard_R40" % class_to_name[curcls]] = mAP3d_R40[j, 2, 0]
                ret_dict["%s_bev/easy_R40" % class_to_name[curcls]] = mAPbev_R40[
                    j, 0, 0
                ]
                ret_dict["%s_bev/moderate_R40" % class_to_name[curcls]] = mAPbev_R40[
                    j, 1, 0
                ]
                ret_dict["%s_bev/hard_R40" % class_to_name[curcls]] = mAPbev_R40[
                    j, 2, 0
                ]
                ret_dict["%s_image/easy_R40" % class_to_name[curcls]] = mAPbbox_R40[
                    j, 0, 0
                ]
                ret_dict["%s_image/moderate_R40" % class_to_name[curcls]] = mAPbbox_R40[
                    j, 1, 0
                ]
                ret_dict["%s_image/hard_R40" % class_to_name[curcls]] = mAPbbox_R40[
                    j, 2, 0
                ]
